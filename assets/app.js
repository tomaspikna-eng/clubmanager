(() => {
  ...
  const route = (name = '') => rootPath + name.replace(/^\/+/, '');
  ...
  const api = {
    ...
    async requireAccess() {
      ...
      if (error) throw error;
      if (!data?.allowed) {
        document.body.innerHTML = `<main class="access-denied"><section><div class="brand-mark">C</div><h1>Club Manager vyžaduje Ultra</h1><p>Tento modul je dostupný pre program Ultra, Elite a administrátora.</p><a href="https://connectsportspro.com/premium">Zobraziť programy</a><button id="logoutDenied">Odhlásiť sa</button></section></main>`;
    ...
    currentSeconds(session) {
      ...
      return base + Math.max(0, Math.floor((Date.now() - new Date(session.started_at).getTime()) / 1000));
    },
